...
def main():
    ...
    while True:
        ...
        if rel[0] is not 0:
            ...
            ax = (axis * math.radians(rotAngle) * delta_time)
        ...
        # Movement Stuff
        if keys[pygame.K_w]:
            scene.camera.position += scene.camera.forward() * delta_time
        if keys[pygame.K_s]:
            scene.camera.position += scene.camera.forward() * delta_time
        if keys[pygame.K_a]:
            scene.camera.position += scene.camera.right() * delta_time
        if keys[pygame.K_d]:
            scene.camera.position += scene.camera.right() * delta_time
