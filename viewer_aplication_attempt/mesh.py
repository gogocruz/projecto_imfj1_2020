...
class Mesh:
        ...
        try:
            ...
            for line in f:
                if line [:2] == "v ":
                    ...
                    if line.find("/")!=-1:
                        faces.append(Vector3(int(face[0][:face[0].find("/")], int(face[1][:face[1].find("/")]), int(face[1][:face[1].find("/")])))
    ...
    for face in faces:
        Mesh.create_tri(vertices[face.x - 1], vertices[faces.y - 1], vertices[face.z - 1], mesh)
